...
int login()
{
    ...
    float cgpa;
    string name, password, inName, inPassword, registerName, registerPassword , registerRePassword ,roll_no ,email,gender,alumni_academic,occupation,department;
    string job ,alumni_graduation;
    ...
    while (1)
    {
        ...
        else if (choice == 1) // Register
        {
            ...
            //have to impose this user already exists restriction.
            cout<<"E-mail address: ";
            fflush(stdin);
            getline(cin,email);
            cout<<"Contact no. : ";
            fflush(stdin);
            getline(cin,contact_number); //have to check data type of number.
            cout<<"Gender: ";
            fflush(stdin);
            getline(cin,gender);
//            cout<<"Society: ";
//            getline(cin,society);
            ...
            if(status == 1)
            {
            	cout<<"Roll no. : (e.g: 21K-XXXX)";
            	fflush(stdin);
            	getline(cin,roll_no);
            	cout<<"CGPA: ";
            	fflush(stdin);
            	getline(cin,cgpa);  //do we have to impose restriction on below 2 cgpa?	
            	cout<<"Department: ";
                fflush(stdin);
            ...
            else
            {
            	...
            	//control back to reenter password.
			}
        }
